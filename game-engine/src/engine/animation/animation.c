...
size_t animation_definition_create(Sprite_Sheet *sprite_sheet, float duration, u8 row, u8 *columns, u8 frame_count)
{
...
size_t animation_create(size_t animation_definition_id, bool does_loop)
{
    ...
    if (adef == NULL)
    {
        ERROR_EXIT("Animation avec id %zu introuvable.", animation_definition_id);
    }

    // Chercher premier espace disponible
...
void animation_render(Animation *animation, vec2 position, vec4 color, u32 texture_slots[32])
{
