...
u32 render_shader_create(const char *path_vert, const char *path_frag)
{
    ...
    if (!file_vertex.is_valid)
        ERROR_EXIT("Erreur lors de la lecture du Shader : %s\n", path_vert);

    u32 shader_vertex = glCreateShader(GL_VERTEX_SHADER); // Création Shader
    ...
    if (!success)
    {
        ...
        ERROR_EXIT("Erreur compilation shader vertex : %s\n", log);
    }
    ...
    u32 shader_fragment = glCreateShader(GL_FRAGMENT_SHADER); // Création Shader FRAGMENT
    ...
    if (!success)
    {
        ...
        ERROR_EXIT("Erreur compilation shader fragment : %s\n", log);
    }
    ...
    if (!success)
    {
        ...
        ERROR_EXIT("Erreur lors de la liaison du shader. %s\n", log);
    }
