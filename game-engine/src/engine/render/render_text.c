...
void render_text(const char *text, float x, float y, vec4 color, u8 is_centered, float text_size)
{
    ...
    // Adjust size based on the given text size
    x *= text_size;
    y *= text_size;
    ...
    if (is_centered)
    {
        ...
        width *= text_size; // Adjust width based on text size
        x -= width * 0.5;
    }
    ...
    for (const char *p = text; *p; ++p)
    {
        ...
        // Adjust position and size based on text size
        float x2 = x + cd.left * text_size;
        float y2 = y - (cd.height - cd.top) * text_size;
        float w = cd.width * text_size;
        float h = cd.height * text_size;
